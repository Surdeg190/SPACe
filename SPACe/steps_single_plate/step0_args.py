...
def sort_key_for_imgs(file_path, sort_purpose, plate_protocol):
    ...
    if plate_protocol == "greiner" or plate_protocol == "perkinelmer":
        ...
        """
        If the name of the image doesn't match any of the examples provided or if you're using a different way to name 
        the images, you'll need to customize this part of the code to make sure it reads the images correctly. 
        This section of code is designed to extract specific information from the image filename, such as the well ID, 
        field of view (FOV), and channel
       
       
        elif [insert the plate protocol] in plate_protocol:
            img filename example:
            .../P000025-combchem-v3-U2OS-24h-L1-copy1/
            P000025-combchem-v3-U2OS-24h-L1-copy1_B02_s5_w3C00578CF-AD4A-4A29-88AE-D2A2024F9A92.tif
            folder = file_path.parents[1].stem
            filename = file_path.stem
            split = [filename.split("_") in case the name contains underscores,
                     filename.split("-") in case the name contains dashes,
                     do not use plit function if the name does not contain special characters.]  
            # here the split is as follow: split= [copy1, B02, s5, w3C00578CF]  copy1 is split[0]
            # B02 is split[1], s5 is split[2], w3C00578CF is split[3]
            well_id = split[1].upper()   # Extract well ID from the second component and convert it to uppercase
            print("well_id", well_id)
            fov = split[2].upper() # Extract FOV from the third component and convert it to uppercase
            print("fov", fov)
            channel = split[3][0:2] # Extract the channel from the fourth component, 
...
class Args(object):
    ...
    def __init__(
        ...
        self.args.main_path = Path(main_path)
        ...
        if self.args.analysis_part == 1:
            ...
            if "perkinelmer" in self.args.plate_protocol or \
                ...
                """ 
                Here is an example of how to add your protocol. You only need to modify the variables in the brackets.
                You can refer to the examples above. The experiment name has to be part of the name of the images folder, 
                i.e., elif "cimini" (images folder name: Jump_Consortium_Datasets_cimini_AssayPlate).
                
                
                # elif [insert the plate protocol] in self.args.imgs_fold.lower():
                #     self.args.img_suffix = [insert the image type, either tiff or tif]
                #     self.args.plate_protocol = self.args.imgs_fold
                #     self.args.img_filepaths = list(self.args.imgs_dir.rglob(f"*.{self.args.img_suffix}"))
