...
class WellAggFeaturesDataset(TensorDataset):
    ...
    def __getitem__(self, grp_idx):
        ...
        # print(grp_val, self.features["Misc_Count_# nucleoli"])
        ...
        # print("Features After torch.as_tensor")
        # print(grp_val,features[0][0])
        if self.transform:
            features = self.transform(features)
        # print("Features After transform")
        # print(grp_val,features[0])
        # print(type(features))
        # print(type(features_no))
        #print(features_no)
        return features#, features_no
        # print(features)
...
class WellAggFeaturesDatasetNo(TensorDataset):
    grp_col = "well-id"
    shuffle = False
    # start_feat_col_index=10
    # min_fov_cell_count=20
    ...
    def __init__(self, features_no, feat_cols, transform=None):
        super().__init__()
        ...
        # get metadata value for each well
        # preserving the original order of the group elements with sort=False,
        self.meta_data = self.features_no.groupby(self.grp_col, sort=False)[self.meta_cols].first().reset_index()
        self.cell_counts = self.features_no[[self.grp_col, self.meta_cols[0]]].groupby(
            self.grp_col, sort=False).count().reset_index()
        self.cell_counts = self.cell_counts.rename(columns={self.meta_cols[0]: "cell-count"})["cell-count"]
        self.meta_data = pd.concat((self.meta_data, self.cell_counts), axis=1)
        self.transform = transform
    ...
    def __getitem__(self, grp_idx):
        ...
        # print("Features before sorting")
        # print(grp_val, self.features["Misc_Count_# nucleoli"])
        features_no = self.features_no.loc[self.features_no[self.grp_col] == grp_val, self.feat_cols].to_numpy()
        features_no = torch.as_tensor(np.float32(features_no))
        # print("Features After torch.as_tensor")
        # print(grp_val,features[0][0])
        if self.transform:
            features = self.transform(features_no)
        # print("Features After transform")
        # print(grp_val,features[0])
        # print(type(features))
        # print(type(features_no))
...
class StatsModel(nn.Module):
    ...
    def __init__(self, feat_cols):
        super().__init__()
        # Distance is calculated from the anchor features
        self.metrics = ("mean", "median") #, "standard deviation", "mad")
        # thresholds = np.linspace(0, 1, 100)
        # self.n_thresholds = len(thresholds)
        # self.register_buffer("thresholds", torch.as_tensor(np.float32(thresholds)))
        # self.register_buffer("anchor_features", torch.as_tensor(np.float32(anchor_features[feat_cols].to_numpy().T)))
    ...
    def forward(self, x):
        ...
        median_reg = torch.median(x, dim=1)[0]
        mean_reg = torch.mean(x, dim=1)
        return mean_reg, median_reg#, std_reg
class WellAggFeatureDistanceMetrics:
    ...
    def __init__(self, args):
        ...
        self.features, self.features_no, self.cell_count_wellwise, self.start_index = self.load_and_preprocess_features()
        ...
        self.features_no = self.sort_rows_fn(self.features_no)
        # print("Before self.normalize_features()")
        # print(self.features["Misc_Count_# nucleoli"])
        ...
        # print("After self.normalize_features()")
        # print(type(self.features_no))
        ...
        self.dataset2 = WellAggFeaturesDatasetNo(self.features_no, self.feat_cols, transform=None)
        # print(self.features)
        # print(self.features_no["Misc_Count_# nucleoli"])
        ...
        self.data_loader2 = DataLoader(
            self.dataset2,
            batch_size=self.batch_size,
            shuffle=False,
            num_workers=0,
            pin_memory=torch.cuda.is_available(),
            collate_fn=self.dataset2.collate_fn_wellwise)
    ...
    def step5_main_run_loop(self):
        ...
        wellwise_stat_metrics = self.get_well_based_feature_stat_metrics()
        writer = pd.ExcelWriter(self.save_path/"wellwise-summary-distance-metrics.xlsx", engine="xlsxwriter")
        writer1 = pd.ExcelWriter(self.save_path/"wellwise-summary-stats-metrics.xlsx", engine="xlsxwriter")
        for _, key in enumerate(self.distance_keys):
            ...
            counter += 1
        writer.close()
        for _, key in enumerate(self.regular_keys):
            val2 = pd.DataFrame(wellwise_stat_metrics[key], columns=self.feat_cols)
            val2 = pd.concat([self.dataset.meta_data, val2], axis=1)
            val2.to_excel(writer1, sheet_name=f"{counter}_{key}", index=False)
            counter2 += 1
        writer1.close()
    ...
    def get_anchor_features(self, features):
        ...
        # print(type(self.args.anchor_density), self.args.anchor_density)
        # print(type(self.args.anchor_cellline), self.args.anchor_cellline)
        # print(type(self.args.anchor_treatment),self.args.anchor_treatment)
        # print(type(self.args.anchor_dosage), self.args.anchor_dosage)
        # # print(anchor_cond[anchor_cond == True])
        # print(features.iloc[:,0:7])
        # print(features.iloc[0,4])
        # print(type(features.iloc[0, 4]))
        # print(features["cell-line"])
        # print(type(features["cell-line"].astype('string')))
    ...
    def normalize_features(self, ):
        ...
        # median_set = np.median(self.features[self.feat_cols].to_numpy(), axis=0)
        # print(self.feat_cols[9])
        # print(median_set)
        # print(len(median_set))
        # print(type(median_set))
    ...
    def get_well_based_feature_summary_distance_metrics(self, ):
        ...
        # stats_model = StatsModel(self.feat_cols)
        # stats_model.to(self.device)
        # stats_model.eval()

        batch_size = self.data_loader.batch_size
        # batch_size2 = self.data_loader2.batch_size2
        N, M1, M2 = self.dataset.__len__(), len(self.dataset.feat_cols), dist_model.n_thresholds
        # N1, M3 = self.dataset2.__len__(), len(self.dataset2.feat_cols)
        # print(N, M1, M2)
        ...
        # stats_wellwise = {key: np.zeros((N1, M3), dtype=np.float32) for key in self.regular_keys}
        with torch.no_grad():
            for ii, features_batch in enumerate(self.data_loader):
                ...
                for jj in tqdm(range(count)):
                    # print(features_batch[jj])
                    features = torch.transpose(features_batch[jj], 0, 1).to(self.device)
                    roc_curve, median_dist, wass_dist, mean_dist, median_reg, mean_reg = dist_model(features)
                    roc_curve, median_dist, wass_dist, mean_dist, median_reg, mean_reg = \
                        roc_curve.cpu().numpy(), \
                            median_dist.cpu().numpy(), \
                            wass_dist.cpu().numpy(), \
                            mean_dist.cpu().numpy(), \
                            median_reg.cpu().numpy(), \
                            mean_reg.cpu().numpy(), \
    ...
    def get_well_based_feature_stat_metrics(self, ):
        ...
        with torch.no_grad():
            ...
            for iii, features_batch2 in enumerate(self.data_loader2):
                # print(ii)
                count2 = len(features_batch2)
                start2 = iii * batch_size
                end = start2 + count2
                index2 = start2
                for jjj in tqdm(range(count2)):
                    # print(features_batch2[jjj])
                    features_no = torch.transpose(features_batch2[jjj], 0, 1).to(self.device)
                    mean_reg, median_reg = stats_model(features_no)
                    mean_reg, median_reg = \
                        mean_reg.cpu().numpy(), \
                            median_reg.cpu().numpy(), \

                    stats_wellwise["mean"][index2] = mean_reg
                    stats_wellwise["median"][index2] = median_reg
                    index2 += 1
                    # break

        torch.cuda.empty_cache()
        # it is easier to calculate the rest r"MIeceiver operating characteristic curves type metrics
        # with numpy rather than torch
        return stats_wellwise
    ...
    def load_and_preprocess_features(self, ):
        ...
        features = pd.concat(
            ...
            axis=1)
        features_no = pd.concat(
            [metadata_features, misc_features, shape_features, intensity_features, texture_features],
            axis=1)
        # print(features)
        # removing Nans
        features.dropna(axis=0, inplace=True)
        features_no.dropna(axis=0, inplace=True)
        ...
        if "exp-id" in list(features_no.columns):
            features_no["exp-id"] = self.load_path.parents[0].stem
        ##############################################################################################################
        ...
        if 'index' in cell_count_wellwise.columns:
            cell_count_wellwise = cell_count_wellwise.rename(columns={"index": "well-id", "well-id": "count"})
        print("cell_count_wellwise", cell_count_wellwise)
        print("Columns:", cell_count_wellwise.columns)
        print("Column Names and Data Types:")
        print(cell_count_wellwise.dtypes)
        cell_count_wellwise.rename(columns={"count": "cell-count"}, inplace=True)
        print("Columns 1:", cell_count_wellwise.columns)
        print("Column Names and Data Types 1:")
        ...
        cell_count_wellwise_no = features_no["well-id"].value_counts().reset_index()
        if 'index' in cell_count_wellwise_no.columns:
            cell_count_wellwise_no = cell_count_wellwise_no.rename(columns={"index": "well-id", "well-id": "count"})
        cell_count_wellwise_no.rename(columns={"count": "cell-count"}, inplace=True)
        # enough_cell_count_well_ids = (cell_count_wellwise["well-id"].loc[
        #     cell_count_wellwise["cell-count"] >= self.args.min_well_cell_count]).to_list()
        enough_cell_count_well_ids_no = (cell_count_wellwise_no["well-id"].loc[
            cell_count_wellwise_no["cell-count"] >= self.min_well_cell_count]).to_list()
        features_no = features_no.iloc[np.isin(features_no["well-id"], enough_cell_count_well_ids_no)]
        ############################################################################################################
        ...
        return features, features_no, cell_count_wellwise, start_index_feat_cols
